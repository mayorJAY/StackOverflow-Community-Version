...
import com.josycom.mayorjay.flowoverstack.data.mapper.toAnswer
...
class AnswerPagingSource(
    private val questionId: Int,
    private val order: String,
    private val sortCondition: String,
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Answer> {
        ...
        return try {
            val response = apiService.getAnswersToQuestion(
                questionId,
                order,
                sortCondition,
                site,
            ...
            val responseItems: List<Answer> = response.items.map { it.toAnswer() }
            val nextKey =
                if (responseItems.isEmpty() || responseItems.size <= AppConstants.PAGE_SIZE) {
                    null
                } else {
